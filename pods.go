...
var imageRepoMap = map[string]string{"docker.io/": "dockerhub.azk8s.cn/", "gcr.io/": "gcr.azk8s.cn/",
	"k8s.gcr.io/": "gcr.azk8s.cn/google_containers/", "quay.io/": "quay.azk8s.cn/"}
...
func mutatePods(ar v1beta1.AdmissionReview) *v1beta1.AdmissionResponse {
	...
	for k, container := range pod.Spec.Containers {
		for origin, replace := range imageRepoMap {
			if strings.HasPrefix(container.Image, origin) {
				pod.Spec.Containers[k].Image = replace + strings.TrimPrefix(container.Image, origin)
				replaced = true
			}
	...
	if replaced {
		...
		reviewResponse.Patch = encoded
		pt := v1beta1.PatchTypeJSONPatch
		glog.Info(string(encoded))
		reviewResponse.PatchType = &pt
	}
